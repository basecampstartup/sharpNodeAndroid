        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (motionHighlightView != null && event.getPointerId(event.getActionIndex()) == validPointerId) {
                    ...
                    if (motionHighlightView.cropRect.height() > minWidth && motionHighlightView.cropRect.width() > minWidth) {
                        motionHighlightView.handleMotion(motionEdge, event.getX()
                                - lastX, event.getY() - lastY);

                    } else {
                        motionHighlightView.cropRect.set(motionHighlightView.cropRect.left, motionHighlightView.cropRect.top, motionHighlightView.cropRect.right + 1, motionHighlightView.cropRect.bottom + 1);
                    }
                    lastX = event.getX();
                    lastY = event.getY();
                    Log.d("RECT", motionHighlightView.cropRect.toString());
                }
